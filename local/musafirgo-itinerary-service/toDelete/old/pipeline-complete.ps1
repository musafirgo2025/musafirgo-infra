...
# Step 7: Generate Excel report using ImportExcel module
function Step-GenerateExcelReport {
    ...
    try {
        ...
        if (-not (Get-Module -ListAvailable -Name ImportExcel)) {
            Write-Log "Installing ImportExcel module..." "INFO"
            try {
                ...
                Write-Log "Failed to install ImportExcel module: $($_.Exception.Message)" "ERROR"
                Write-Log "Falling back to CSV report..." "WARNING"
                return Step-GenerateCSVReport
            }
        }
        ...
        $filename = "MusafirGO_Pipeline_Report_$timestamp.xlsx"
        $filepath = Join-Path $PSScriptRoot $filename
        
        # Create data arrays for different sheets
        $pipelineSummary = @()
        $pipelineSummary += [PSCustomObject]@{
            "Élément" = "Heure de début"
            "Valeur" = $Global:PipelineResults.StartTime.ToString('yyyy-MM-dd HH:mm:ss')
            "Statut" = "OK"
            "Durée (s)" = 0
            "Détails" = "Aucune erreur"
        }
        $pipelineSummary += [PSCustomObject]@{
            "Élément" = "Heure de fin"
            "Valeur" = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
            "Statut" = "OK"
            "Durée (s)" = 0
            "Détails" = "Aucune erreur"
        }
        $pipelineSummary += [PSCustomObject]@{
            "Élément" = "Durée totale"
            "Valeur" = "$([math]::Round($Global:PipelineResults.TotalDuration, 2)) secondes"
            "Statut" = "OK"
            "Durée (s)" = 0
            "Détails" = "Aucune erreur"
        }
        $pipelineSummary += [PSCustomObject]@{
            "Élément" = "Statut global"
            "Valeur" = if ($Global:PipelineResults.Success) { "SUCCÈS" } else { "ÉCHEC" }
            "Statut" = if ($Global:PipelineResults.Success) { "OK" } else { "FAIL" }
            "Durée (s)" = 0
            "Détails" = "Aucune erreur"
        }
        ...
        # Create steps data
        $stepsData = @()
        foreach ($stepName in $Global:PipelineResults.Steps.Keys) {
            $step = $Global:PipelineResults.Steps[$stepName]
            $stepsData += [PSCustomObject]@{
                "Étape" = $stepName
                "Statut" = if ($step.Success) { "Réussi" } else { "Échoué" }
                "Code" = if ($step.Success) { "OK" } else { "FAIL" }
                "Durée (s)" = [math]::Round($step.Duration, 2)
                "Détails" = if ($step.Error) { $step.Error } else { "Aucune erreur" }
            }
        }
        
        # Create API test results data
        $apiData = @()
        if ($Global:PipelineResults.Steps.ContainsKey("APITests") -and $Global:PipelineResults.Steps["APITests"].Result) {
            $apiResults = $Global:PipelineResults.Steps["APITests"].Result
            $apiData += [PSCustomObject]@{
                "Métrique" = "Total des tests"
                "Valeur" = $apiResults.Total
                "Statut" = "OK"
                "Code" = "OK"
                "Détails" = "Aucune erreur"
            }
            $apiData += [PSCustomObject]@{
                "Métrique" = "Tests réussis"
                "Valeur" = $apiResults.Passed
                "Statut" = "OK"
                "Code" = "OK"
                "Détails" = "Aucune erreur"
            }
            $apiData += [PSCustomObject]@{
                "Métrique" = "Tests échoués"
                "Valeur" = $apiResults.Failed
                "Statut" = if ($apiResults.Failed -eq 0) { "OK" } else { "FAIL" }
                "Code" = if ($apiResults.Failed -eq 0) { "OK" } else { "FAIL" }
                "Détails" = "Aucune erreur"
            }
            $apiData += [PSCustomObject]@{
                "Métrique" = "Taux de réussite"
                "Valeur" = "$([math]::Round(($apiResults.Passed / $apiResults.Total) * 100, 2))%"
                "Statut" = "OK"
                "Code" = "OK"
                "Détails" = "Aucune erreur"
            }
        ...
        $perfData = @()
        if ($Global:PipelineResults.Steps.ContainsKey("PerformanceTests") -and $Global:PipelineResults.Steps["PerformanceTests"].Result) {
            $perfResults = $Global:PipelineResults.Steps["PerformanceTests"].Result
            $validResults = $perfResults.Values | Where-Object { $_ -ge 0 }
            if ($validResults.Count -gt 0) {
                $averageResponseTime = ($validResults | Measure-Object -Average).Average
                $maxResponseTime = ($validResults | Measure-Object -Maximum).Maximum
                $minResponseTime = ($validResults | Measure-Object -Minimum).Minimum
                
                $perfData += [PSCustomObject]@{
                    "Métrique" = "Temps de réponse moyen"
                    "Valeur" = "$([math]::Round($averageResponseTime, 2)) ms"
                    "Statut" = "OK"
                    "Code" = "OK"
                    "Détails" = "Aucune erreur"
                }
                $perfData += [PSCustomObject]@{
                    "Métrique" = "Temps de réponse maximum"
                    "Valeur" = "$([math]::Round($maxResponseTime, 2)) ms"
                    "Statut" = "OK"
                    "Code" = "OK"
                    "Détails" = "Aucune erreur"
                }
                $perfData += [PSCustomObject]@{
                    "Métrique" = "Temps de réponse minimum"
                    "Valeur" = "$([math]::Round($minResponseTime, 2)) ms"
                    "Statut" = "OK"
                    "Code" = "OK"
                    "Détails" = "Aucune erreur"
                }
                $perfData += [PSCustomObject]@{
                    "Métrique" = "Tests de performance réussis"
                    "Valeur" = "$($validResults.Count)/$($perfResults.Count)"
                    "Statut" = "OK"
                    "Code" = "OK"
                    "Détails" = "Aucune erreur"
                }
        ...
        $endpointsData = @()
        $endpoints = @(
            @{Endpoint="GET /api/itineraries"; Description="Lister tous les itinéraires"},
            @{Endpoint="POST /api/itineraries"; Description="Créer un nouvel itinéraire"},
            @{Endpoint="GET /api/itineraries/{id}"; Description="Obtenir un itinéraire spécifique"},
            @{Endpoint="PUT /api/itineraries/{id}"; Description="Mettre à jour un itinéraire"},
            @{Endpoint="DELETE /api/itineraries/{id}"; Description="Supprimer un itinéraire"},
            @{Endpoint="GET /api/itineraries?city={city}"; Description="Rechercher par ville"},
            @{Endpoint="GET /api/itineraries?from={date}&to={date}"; Description="Rechercher par dates"},
            @{Endpoint="GET /api/itineraries?page={page}&size={size}"; Description="Pagination"},
            @{Endpoint="POST /api/itineraries/{id}/days/{day}/items"; Description="Ajouter un élément"},
            @{Endpoint="DELETE /api/itineraries/{id}/days/{day}/items/{index}"; Description="Supprimer un élément"},
            @{Endpoint="POST /api/v1/itineraries/{id}/media"; Description="Télécharger un fichier"},
            @{Endpoint="GET /api/v1/itineraries/{id}/media"; Description="Obtenir tous les médias"},
            @{Endpoint="GET /api/v1/itineraries/{id}/media/active"; Description="Obtenir les médias actifs"},
            @{Endpoint="GET /api/v1/itineraries/{id}/media/paged"; Description="Médias avec pagination"},
            @{Endpoint="GET /api/v1/itineraries/{id}/media/{mediaId}"; Description="Obtenir un média spécifique"},
            @{Endpoint="DELETE /api/v1/itineraries/{id}/media/{mediaId}"; Description="Supprimer un média"},
            @{Endpoint="DELETE /api/v1/itineraries/{id}/media"; Description="Supprimer tous les médias"},
            @{Endpoint="POST /api/v1/itineraries/{id}/media/{mediaId}/sas"; Description="Générer URL SAS"},
            @{Endpoint="GET /actuator/health"; Description="Santé de l'application"},
            @{Endpoint="GET /actuator/health/db"; Description="Santé de la base de données"},
            @{Endpoint="GET /actuator/health/redis"; Description="Santé de Redis"},
            @{Endpoint="GET /actuator/info"; Description="Informations sur l'application"},
            @{Endpoint="GET /actuator/metrics"; Description="Métriques disponibles"},
            @{Endpoint="GET /swagger-ui.html"; Description="Interface Swagger UI"},
            @{Endpoint="GET /v3/api-docs"; Description="Documentation OpenAPI"}
        )
        
        foreach ($endpoint in $endpoints) {
            $endpointsData += [PSCustomObject]@{
        ...
        # Create the Excel file with multiple worksheets
        $pipelineSummary | Export-Excel @excelParams -WorksheetName "Résumé Pipeline"
        $stepsData | Export-Excel @excelParams -WorksheetName "Détails Étapes" -Append
        $apiData | Export-Excel @excelParams -WorksheetName "Tests API" -Append
        $perfData | Export-Excel @excelParams -WorksheetName "Performance" -Append
        $endpointsData | Export-Excel @excelParams -WorksheetName "Endpoints" -Append
        
        Write-Log "Rapport Excel généré avec succès : $filename" "SUCCESS"
        Write-Log "Chemin du fichier : $filepath" "INFO"
        ...
        Write-Log "Tentative de génération d'un rapport CSV de secours..." "WARNING"
        return Step-GenerateCSVReport
    }
}

# Fallback function for CSV report
function Step-GenerateCSVReport {
    Write-Log "Generating CSV report as fallback..." "INFO"
    
    try {
        # Generate filename with timestamp
        $timestamp = Get-Date -Format 'yyyyMMdd_HHmmss'
        $filename = "MusafirGO_Pipeline_Report_$timestamp.csv"
        $filepath = Join-Path $PSScriptRoot $filename
        ...
        $csvContent += "=== RÉSUMÉ DU PIPELINE ==="
        $csvContent += "Élément,Valeur,Statut,Durée (s),Détails"
        $csvContent += "Heure de début,$($Global:PipelineResults.StartTime.ToString('yyyy-MM-dd HH:mm:ss')),OK,0,Aucune erreur"
        $csvContent += "Heure de fin,$((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')),OK,0,Aucune erreur"
        $csvContent += "Durée totale,$([math]::Round($Global:PipelineResults.TotalDuration, 2)) secondes,OK,0,Aucune erreur"
        $csvContent += "Statut global,$(if ($Global:PipelineResults.Success) { 'SUCCÈS' } else { 'ÉCHEC' }),$(if ($Global:PipelineResults.Success) { 'OK' } else { 'FAIL' }),0,Aucune erreur"
        $csvContent += ""
        
        # Add step details
        $csvContent += "=== DÉTAILS DES ÉTAPES ==="
        $csvContent += "Étape,Statut,Code,Durée (s),Détails"
        foreach ($stepName in $Global:PipelineResults.Steps.Keys) {
            $step = $Global:PipelineResults.Steps[$stepName]
            $status = if ($step.Success) { "Réussi" } else { "Échoué" }
            $code = if ($step.Success) { "OK" } else { "FAIL" }
            $duration = [math]::Round($step.Duration, 2)
            $details = if ($step.Error) { $step.Error } else { "Aucune erreur" }
            $csvContent += "$stepName,$status,$code,$duration,$details"
        }
        
        # Write CSV file
        $csvContent | Out-File -FilePath $filepath -Encoding UTF8
        
        Write-Log "Rapport CSV généré avec succès : $filename" "SUCCESS"
        Write-Log "Chemin du fichier : $filepath" "INFO"
        ...
        Write-Log "Erreur lors de la génération du rapport CSV : $($_.Exception.Message)" "ERROR"
        return @{
            Success = $false
