...
func (p *MusafirGoPipeline) InitializeDatabase() bool {
	...
	p.Log("INFO", "Stopping existing containers...")
...
func (p *MusafirGoPipeline) LoadTestData() bool {
	...
	cmd := exec.Command("docker-compose", "exec", "-T", "postgres", "psql", "-U", "itinerary", "-d", "itinerary", "-f", "/docker-entrypoint-initdb.d/dump-data.sql")
	if err := cmd.Run(); err != nil {
...
func (p *MusafirGoPipeline) APITests() *APITestResult {
	...
	endpoints := []struct {
		...
		{"DELETE", "/api/v1/itineraries/{id}/media/{mediaId}", "Delete specific media", "", 200, "Media"},
		{"DELETE", "/api/v1/itineraries/{id}/media", "Delete all media", "", 200, "Media"},
	...
	// Variables pour les tests dynamiques
	var createdItineraryID string
	...
	for _, endpoint := range endpoints {
		...
		if strings.Contains(url, "{id}") {
			if createdItineraryID == "" {
				// Créer un itinéraire de test d'abord
				createResp, err := http.Post(p.BaseURL+"/api/itineraries", "application/json", 
					strings.NewReader(`{"city":"Test City","startDate":"2025-04-01","endDate":"2025-04-03","days":[{"day":1,"items":["Test activity"]}]}`))
				if err == nil && createResp.StatusCode == 201 {
					var createResult map[string]interface{}
					json.NewDecoder(createResp.Body).Decode(&createResult)
					createdItineraryID = createResult["id"].(string)
					createResp.Body.Close()
				}
			}
			url = strings.ReplaceAll(url, "{id}", createdItineraryID)
		}
		if strings.Contains(url, "{mediaId}") {
			if createdMediaID == "" && createdItineraryID != "" {
				// Créer un média de test
				mediaResp, err := http.Post(p.BaseURL+"/api/v1/itineraries/"+createdItineraryID+"/media", "multipart/form-data", 
					strings.NewReader("test file content"))
				if err == nil && mediaResp.StatusCode == 201 {
					var mediaResult map[string]interface{}
					json.NewDecoder(mediaResp.Body).Decode(&mediaResult)
					createdMediaID = mediaResult["id"].(string)
					mediaResp.Body.Close()
				}
			}
			url = strings.ReplaceAll(url, "{mediaId}", createdMediaID)
		}
	...
	if createdItineraryID != "" {
		req, _ := http.NewRequest("DELETE", p.BaseURL+"/api/itineraries/"+createdItineraryID, nil)
...
func (p *MusafirGoPipeline) PerformanceTests() *PerformanceResult {
	...
	if result.HealthCheck >= 0 { validResults = append(validResults, result.HealthCheck) }
	if result.ListItineraries >= 0 { validResults = append(validResults, result.ListItineraries) }
	if result.SearchByCity >= 0 { validResults = append(validResults, result.SearchByCity) }
	if result.ActuatorInfo >= 0 { validResults = append(validResults, result.ActuatorInfo) }
	if result.ActuatorMetrics >= 0 { validResults = append(validResults, result.ActuatorMetrics) }
	if result.SwaggerUI >= 0 { validResults = append(validResults, result.SwaggerUI) }
	if result.OpenAPIDocs >= 0 { validResults = append(validResults, result.OpenAPIDocs) }
	if result.CreateItinerary >= 0 { validResults = append(validResults, result.CreateItinerary) }
	if result.UpdateItinerary >= 0 { validResults = append(validResults, result.UpdateItinerary) }
	if result.DeleteItinerary >= 0 { validResults = append(validResults, result.DeleteItinerary) }
